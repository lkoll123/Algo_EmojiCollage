...
def emojiRGB(inputarr, pathToPics):
  ...
  for filename in os.listdir(pathToPics):
    #using cropSquare function to crop emoji to emojiSizexemojiSize Pixels size, and assigning the resized image to variable resizedimage
    resizedimage = cropSquare(filename, emojiSize, pathToPics)
    resizedimage = Image.open(pathToPics + '\\' + filename)
...
#Using cropMultiple function to crop image to a square that is a multiple of the emoji width and height(emojiSize)
cropMultiple(fileName, emojiSize, pathTwo)
...
for x in range(0, int(mainImage.width/emojiSize)):
    for y in range(0, int(mainImage.height/emojiSize)):
        ...
        avgval = avgRGB(fileName, emojiSize * x, emojiSize * y, emojiSize, emojiSize, pathTwo)
        #Getting the closest RGB value in array averageRGB(holds all of the average RGBS of the emojis), to the avgval found in the given emojiSize*emojiSize square of pixels in image fileName
